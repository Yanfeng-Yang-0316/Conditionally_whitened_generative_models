for seed_idx in range(10):
    ...
    log_file = f"cw_nsdiff_lambda_{lambda_min}_weigen{weigen}_e2e.txt"
    ...
    def p_sample_loop(model, x, x_mark, y_0_hat, gx, y_T_mean, n_steps, alphas, one_minus_alphas_bar_sqrt, alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled, betas_bar, betas_tiled_m_1, betas_bar_m_1):
        ...
        y_p_seq.append(y_0)
        return y_p_seq # list， n_steps+1, shape= (B, T, N)
    ...
    class SigmaEstimation(nn.Module):
        ...
        def __init__(self, seq_len, pred_len, enc_in, hidden_size=512, kernel_size=24):
            ...
            self.mlp = nn.Sequential(
                nn.Linear(seq_len -  kernel_size, hidden_size), #  kernel_size
                nn.ReLU(),
            ...
            self.padding = self.kernel_size // 2  # padding，
    ...
    class ConditionalLinear(nn.Module):
        ...
        def forward(self, x, t):
            ...
            # 2. t
            gamma = self.embed(t)
            # 3， gammareshape (B, 1, num_out)  (B, num_out)
    ...
    class ConditionalGuidedModel(nn.Module):
        def __init__(self, diff_steps, enc_in):
            ...
            n_steps = diff_steps + 1 # 1，t=0
            data_dim = enc_in * 3 # 3：y_t, y_0_hat, g_x
            # 3
    ...
    def _process_train_batch(model,
        ...
        y_sigma = wv_sigma_trailing(torch.concat([batch_x, batch_y], dim=1), rolling_length) # sigma_y0
        ...
        dec_inp = torch.cat([dec_inp_label, dec_inp_pred], dim=1) # label_len0，
        ...
        gx = cond_pred_model_g(batch_x) + EPS # (B, O, N) eps
        loss1 = (y_0_hat_batch - batch_y).square().mean() # f
        loss2 = (torch.sqrt(gx)- torch.sqrt(y_sigma)).square().mean() # g（）
        ...
        y_t_batch = q_sample(batch_y, y_T_mean, model.alphas_bar_sqrt,
                                model.one_minus_alphas_bar_sqrt, t, noise=noise) # y_t_batch
        ...
        loss = kl_loss + loss1 + loss2  # fg（co-training）
        return loss
    ...
    def _process_val_batch(model,
        ...
        # 
        y_0_hat_batch, _ = cond_pred_model(batch_x, batch_x_mark, dec_inp,batch_y_mark_input)
        gx = cond_pred_model_g(batch_x)
        
        preds = []
        # minisample
        for i in range(100 //minisample):
            repeat_n = int(minisample)
            # reshapey_0_hat、batch_x、gx
    ...
    def stabilize_upper(U, eps_diag):
        ...
        diag_new = diag.abs().clamp_min(eps_diag)   # 
        U_stable = U.clone()
    ...
    def whiten_sequence_fast(theta, conditional_mean, upper_mat,
        ...
        theta: [B, T, D]
        conditional_mean: [B, T, D]
        upper_mat: [B, T, D, D],  U,  Sigma ≈ U^T U
    ...
    ###################################################   train   #######################################
        for i, (batch_x,
            ...
            loss_nsdiff = _process_train_batch(model = model,
    ...
    ###################################################   test   #######################################
            ...
            for i, (batch_x,
                ...
                test_loss = (loss_jmce + loss_nsdiff)
            ...
            if val_avg < best_val_loss and step :
                ...
                best_model_state_nsdiff = {k: v.clone() for k, v in model.state_dict().items()}
                best_model_state_nsdiff_cond_mean = {k: v.clone() for k, v in cond_pred_model.state_dict().items()}
                best_model_state_nsdiff_sigma = {k: v.clone() for k, v in cond_pred_model_g.state_dict().items()}
    ...
    if best_model_state_jmce is not None:
        ...
        model.load_state_dict(best_model_state_nsdiff)
        cond_pred_model.load_state_dict(best_model_state_nsdiff_cond_mean)
        cond_pred_model_g.load_state_dict(best_model_state_nsdiff_sigma)
