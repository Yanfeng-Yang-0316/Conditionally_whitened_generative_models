for seed_idx in range(10):
    ...
    config = {
        ...
        # loss weight 
        "matrix_norm_weight": [(7*192)**0.5 * 1e-1, 1, 0.], # [f norm, svd norm, fft norm] fft norm is always 0
        "fft_weight": [1, 0.], # [L2 norm, fft norm] fft norm is always 0
        "eign_penalty": 50, # w_Eigen
        "eps_eign_min": 1e-1, # lambda_min
    ...
    def stabilize_upper(U, eps_diag, verbose: bool = False):
        ...
        Only stabilize the diagonal: clip to at least eps_diag while preserving gradients.
        """

        # Operate on a clone to avoid modifying the original U (otherwise autograd may raise an in-place error).
        U_stable = U.clone()

        # Diagonal view (a view of U_stable).
        ...
        if verbose:
            # Number of clipped entries = positions where the clamped value exceeds the original absolute value.
    ...
    def fm_sampler(net, 
        ...
        for i in range(num_steps):
            ...
            # f(t, x_t): Used in both prediction and correction steps.
    ...
    log_file = f'cw_flowts_lambdamin_{lambda_min}_weigen{weigen}_e2e.txt'
